...
function buildPhases(taskData, reviews) {
  ...
  // Flat task list for the Kanban board: column = normalized status bucket.
  const tasks = [];
  for (const p of phases) {
    for (const t of p.tasks) {
      const col = t.status === "done" ? "done"
        : (t.status === "in-progress" || t.status === "blocked") ? "in-progress"
        : "not-started";
      tasks.push({
        id: t.id, title: t.title, status: t.status, column: col,
        phase_id: p.id, phase_name: p.name,
        in_next_phase: p.id === nextId,
      });
    }
  }
...
// Rewrite a single task's status (and done_on) in the raw TASKS.md text.
// Only the matched row changes; everything else is preserved. Throws if the id
// isn't found so the caller can surface a clear error.
function setTaskStatus(text, id, status, today) {
  const canonical = { done: "done", "in-progress": "in-progress", "not-started": "not-started" };
  const newStatus = canonical[status];
  if (!newStatus) throw new Error("invalid status: " + status);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (cells[0] !== String(id)) continue;

    while (cells.length < 7) cells.push("");   // id|title|phase|status|est|done_on|notes
    cells[3] = newStatus;
    cells[5] = newStatus === "done" ? (cells[5] || today) : "";
    lines[i] = "| " + cells.join(" | ") + " |";
    changed = true;
    break;
  }
...
module.exports = { parseTasks, buildPhases, setTaskStatus };
